...
def create_heatmap (accumulated_exposures):
    #
...
def accumulate_exposures (frame_shape, labels, seconds_per_frame = 1.0):
    accumulated_exposures = np.zeros((frame_shape[0], frame_shape[1]), dtype=np.float)
    ...
    for label in labels:
        cv2.fillConvexPoly(accumulated_exposures, label, (seconds_per_frame))
...
def main():
    ...
    with open('labels.txt') as f:
        ...
        for line in lines:
            ...
            if accumulated_exposures is None:
    ...
    create_heatmap(accumulated_exposures)
